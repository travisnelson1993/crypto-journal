...
from sqlalchemy import Column, Integer, Integer, String, Numeric, Enum, TIMESTAMP, ForeignKey, func
...
class ExecutionMatch(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    open_execution_id = Column(Integer, ForeignKey('executions.id', ondelete='CASCADE'), nullable=False)
    close_execution_id = Column(Integer, ForeignKey('executions.id', ondelete='CASCADE'), nullable=False)
