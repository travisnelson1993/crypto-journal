...
def imported_file_count_by_hash(conn, file_hash):
    with conn.cursor() as cur:
        cur.execute("SELECT count(*) FROM imported_files WHERE file_hash = %s", (file_hash,))
        return cur.fetchone()[0]
...
def test_importer_idempotent_and_records_filename(tmp_path, dsn):
    ...
    assert first_count == expected_rows, f"expected {expected_rows} rows after first import, got {first_count}"
    ...
    assert second_count == first_count, "Importer was not idempotent; row count changed after re-run"
