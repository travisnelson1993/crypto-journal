    ...
    try:
        ...
        if inserts_open:
            ...
            try:
                ...
                if "no unique or exclusion constraint matching the on conflict specification" in msg or isinstance(e, psycopg2.errors.InvalidColumnReference):
                    ...
                    # This handles edge case where last iteration succeeded but cursor state is uncertain
                    if cur.closed:
                        cur = conn.cursor()
                else:
                    raise
        ...
        if inserts_close:
            ...
            for r in inserts_close:
                ...
                # COALESCE preserves existing source_filename from open row if already set
                cur.execute("""
                    UPDATE trades
                    SET end_date = %s,
                        exit_price = %s,
